...
The common gotcha is the OG card. Open Graph images are usually 1.91:1, which is **not** a supported Bloom ratio — map it to `16:9` rather than inventing `1.91:1`. The same rule holds for any platform spec that doesn't line up with the list: pick the nearest supported ratio, don't pass one Bloom can't accept.
